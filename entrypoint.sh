...
function runCdk(){
	echo "Run cdk ${INPUT_CDK_SUBCOMMAND} ${*} \"${INPUT_CDK_STACK}\" --app \"${INPUT_CDK_STACK_SOURCE}\""
	output=$(cdk ${INPUT_CDK_SUBCOMMAND} ${*} "${INPUT_CDK_STACK}" --app "${INPUT_CDK_STACK_SOURCE}" -v 2>&1)
	exitCode=${?}
