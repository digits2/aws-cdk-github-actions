...
function runCdk(){
	echo "Run cdk ${INPUT_CDK_SUBCOMMAND} ${*} --app \"${INPUT_CDK_STACK_SOURCE}\" -v"
	output=$(cdk ${INPUT_CDK_SUBCOMMAND} ${*} "${INPUT_CDK_STACK}" --app \"${INPUT_CDK_STACK_SOURCE}\" --plugin cdk-assume-role-credential-plugin -v 2>&1)
	exitCode=${?}
